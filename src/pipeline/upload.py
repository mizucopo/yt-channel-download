...
def upload_video(video_id: str, local_path: str) -> bool:
...
def upload_next() -> bool:
    """次の待機中の動画をアップロードする.

    Returns:
        アップロード対象があった場合はTrue
    """
    stream = db.get_next_pending("thumbs_done", settings.max_retries)
    if stream is None or stream.local_path is None:
        return False

    return upload_video(stream.video_id, stream.local_path)
