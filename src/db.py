...
def insert_stream(stream: Stream) -> None:
    """新しいストリームを登録する.
...
def get_stream(video_id: str) -> Stream | None:
...
def get_streams_by_status(status: str) -> list[Stream]:
...
def get_next_pending(status: str, max_retries: int) -> Stream | None:
    """次の処理対象を取得する.
